...
PATCHED=$(adb shell 'find /storage/emulated/0/Download -maxdepth 1 -type f -name "*.img" 2>/dev/null | tr -d "\r" | grep -v init_boot | grep -v trashed | tail -1')
...
echo ""
    ...
    echo ""
    info "Installing ONE root manager..."

    local root_installed=0

    for apk in ROOT_MANAGERS/*.apk; do
        [ -e "$apk" ] || continue

        if [ "$root_installed" -eq 1 ]; then
            echo "⏭ Skipping extra root manager: $(basename "$apk")"
            continue
        fi
        ...
        if [ $? -eq 0 ]; then
            root_installed=1
            echo "✓ Root manager installed: $(basename "$apk")"
        fi
    done
...
recover_if_broken() {
    if ! bash -n "$0" 2>/dev/null; then
        echo "⚠ Script corrupted — attempting recovery..."
